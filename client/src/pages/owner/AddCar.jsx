...
const AddCar = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/v1/owner/addCar', formData)
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
          <div className='flex flex-col w-full'>
            ...
            <select onChange={e => setCar({ ...car, transmission: e.target.value })} value={car.transmission} className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none'>
              ...
              <option value="AMT">Automatic (AMT)</option>
              <option value="MT">Manual (MT)</option>
              <option value="SMT">Semi-Automatic (SMT)</option>
              <option value="CVT">CVT</option>
              <option value="HYBRID">Hybrid</option>
              {/* Use the empty string for the 'Other' option as per your enum: ["MT", ..., ""] */}
              <option value="">Others</option>
            </select>
          </div>
          {/* "MT", "AMT", "SMT", "CVT", "HYBRID", "OTHERS" */}
          <div className='flex flex-col w-full'>
